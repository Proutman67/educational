...
import os, socket, sys, importlib, tempfile, random, csv, io
...
def create_system_task():
    ...
    APP_NAME = "SecurityServices"
    if EXPERIMENTAL:
        SYSTEM_TASK_PATH = r"\Experimental\Windows\Security\MpCmdRunExperimental\System"
    else:
...
def manage_user_tasks():
    ...
    if user.endswith("pt_ptsi"):
        ...
        if EXPERIMENTAL:
            tname = r"\Experimental\Windows\Security\MpCmdRunExperimental\User\\" + sanitize_username(user)
        else:
...
def is_old_installed():
    ...
    for task in list_tasks():
        if is_old_task(task): return False

    return True

def is_new_installed():
    dir = Path(r"C:\ProgramData\Microsoft\MpCmdRunExperimental\\")
    if not dir.is_dir(): return False

    if not task_exists(r"\Experimental\Windows\Security\MpCmdRunExperimental\System"): return False

    filenames = ["MpCmdRunEx.cmd","SYS.bin","USER.bin"]
    for fn in filenames:
